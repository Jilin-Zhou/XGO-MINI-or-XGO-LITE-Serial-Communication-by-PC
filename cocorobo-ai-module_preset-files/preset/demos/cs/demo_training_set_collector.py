...
display_cjk_string(img, 10, 10, "图像捕捉助手", font_size=1, color=(0,255,255))
display_cjk_string(img, 10, 35, "用来AI训练目的", font_size=1, color=(0,255,255))
lcd.display(img,oft=(0,0))

time.sleep_ms(1000)
display_cjk_string(img, 10, 190, "测试相机...", font_size=1, color=(255,255,255))
lcd.display(img,oft=(0,0))
...
try:
    ...
    display_cjk_string(img, 10, 215, "相机就绪.", font_size=1, color=(0,255,0))
    lcd.display(img,oft=(0,0))
    time.sleep_ms(1000)
    ...
    display_cjk_string(img, 10, 215, "相机初始化失败.", font_size=1, color=(0,255,0))
    lcd.display(img,oft=(0,0))
    img.clear()
    lcd.display(img,oft=(0,0))
...
while True:
    ...
    if (key_state_left == 1 and ksl == 0):
        ...
        current_shot_taken = 0
        display_cjk_string(img_raw, 0, 0, "类:"+str(current_class)+"/"+str(total_classes), font_size=1, color=(255,255,255))
        ksl = 1
    ...
    if (key_state_right == 1 and ksr == 0):
        ...
        a = img_raw.ai_to_pix()
        img_raw.draw_rectangle(0,200,224,24,color=(0,0,0),fill=True)
        display_cjk_string(img_raw, 0, 204, "保存: "+ str(image_file_name), font_size=1, color=(255,255,0))
    ...
    display_cjk_string(img_raw, 0, 0, "类:"+str(current_class)+"/"+str(total_classes), font_size=1, color=(255,255,255))
    display_cjk_string(img_raw, 0, 204, "拍摄" + str(total_shot_need_tobe_taken) + "张中的第" + str(current_shot_taken) + "张", font_size=1, color=(255,255,255))
