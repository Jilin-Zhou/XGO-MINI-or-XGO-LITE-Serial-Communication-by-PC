...
while(True):
    ...
    if int(DSL_deflection_angle) < 0:
        display_cjk_string(img, 5, 5, "左转", font_size=1, color=(255,255,255))
    elif int(DSL_deflection_angle) > 0:
        # img.draw_string(5, 5, "Turn Right", scale=2, mono_space=False)
        display_cjk_string(img, 5, 5,"右转", font_size=1, color=(255,255,255))
    elif int(DSL_deflection_angle) == 0:
        # img.draw_string(5, 5, "Go Straight", scale=2, mono_space=False)
        display_cjk_string(img, 5, 5, "直走", font_size=1, color=(255,255,255))
