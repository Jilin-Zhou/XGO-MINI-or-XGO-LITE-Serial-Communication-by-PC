...
while True:
    ...
    if train_status == 0:
        if key1.value() == 0 and last_btn_status == 1:
            ...
            if cap_num < class_num:
                ...
                display_cjk_string(img, 0, 0, "press right key to cap "+class_names[cap_num], font_size=1, color=(255,255,255))
                # img = draw_string(img, 0, 2, "press right key to cap "+class_names[cap_num], color=lcd.WHITE,scale=1, bg=lcd.RED)
            elif cap_num < class_num + sample_num:
                img.draw_rectangle(0, 0, 240,20, fill=True, color=(0,0,0))
                display_cjk_string(img, 0, 0, "right key to cap sample{}".format(cap_num-class_num), font_size=1, color=(255,255,255))
    ...
    if train_status == 0:
        if cap_num >= class_num + sample_num:
            print("start train")
            img.draw_rectangle(0,0, 240,20, fill=True, color=(0,0,0))
            display_cjk_string(img, 0, 0, "training...", font_size=1, color=(255,255,255))
        ...
        if res_index >= 0 and min_dist < THRESHOLD :
            ...
            img.draw_rectangle(0,0, 240,20, fill=True, color=(0,0,0))
            display_cjk_string(img, 0, 2, 'maybe {}'.format(class_names[res_index]), font_size=1, color=(255,255,255))
