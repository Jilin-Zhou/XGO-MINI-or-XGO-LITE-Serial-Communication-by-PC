...
display_cjk_string(canvas, 10,10, "Voice Sample", font_size=1, color=(255,255,255))
display_cjk_string(canvas, 10,30, "Collecting Procedure", font_size=1, color=(255,255,255))
...
try:
    while _voice_record == True:
        ...
        for i in range(_voice_keyword_num):
            for j in range(_voice_model_num):
                ...
                canvas.draw_rectangle(10,75, 220, 20, color=(255,255,255), thickness=1, fill=True)

                display_cjk_string(canvas, 10,55, "Recording Progress:", font_size=1, color=(0,0,255))
                display_cjk_string(canvas, 15,77, "Clip " +str(i+1)+ " of " + str(_voice_keyword_num)+ ", Take " +str(j+1)+ " of " + str(_voice_model_num), font_size=1, color=(0,0,0))
                display_cjk_string(canvas, 10,100, "Press Button A to Record", font_size=1, color=(0,0,255))
                display_cjk_string(canvas, 10,180, "Recording State:", font_size=1, color=(0,0,255))
                canvas.draw_rectangle(10,200, 230, 50, color=(0,0,0), thickness=1, fill=True)
                display_cjk_string(canvas, 10,200, "Wait for press...", font_size=1, color=(20,20,20))

                '''
                canvas.draw_string(10,55, "Recording Progress:", color=(0,0,255), scale=2, mono_space=False)
                canvas.draw_string(15,75, "Clip " +str(i+1)+ " of " + str(_voice_keyword_num)+ ", Take " +str(j+1)+ " of " + str(_voice_model_num), color=(0,0,0), scale=2, mono_space=False)
                canvas.draw_string(10,100, "Press Button A to Record", color=(0,0,255), scale=2, mono_space=False)
                canvas.draw_string(10,180, "Recording State:", color=(0,0,255), scale=2, mono_space=False)
                canvas.draw_string(10,200, "Wait for press...", color=(20,20,20), scale=3, mono_space=False)
                '''
                ...
                while (_s_daemon.get_status() != 2):
                    if (_s_daemon.get_status() == 1):
                        print("[CocoRobo] Speak now!")
                        _led_red.value(1)
                        _led_blue.value(1)
                        canvas.draw_rectangle(10,200, 230, 50, color=(0,0,0), thickness=1, fill=True)

                        display_cjk_string(canvas, 10,200, "Speak Now!", font_size=2, color=(255,0,0))
                        # canvas.draw_string(10,200, "Speak Now!", color=(255,0,0), scale=3, mono_space=False)
                        lcd.display(canvas, oft=(_canvas_x,_canvas_y))
                    if (_s_daemon.get_status() == 5):
                        print("[CocoRobo] Gathering noise...")
                        _led_red.value(1)
                        canvas.draw_rectangle(10,200, 230, 50, color=(0,0,0), thickness=1, fill=True)

                        display_cjk_string(canvas, 10,200, "Preparing", font_size=2, color=(255,0,0))
                        # canvas.draw_string(10,200, "Gathering Noise...", color=(255,0,0), scale=3, mono_space=False)
                        lcd.display(canvas, oft=(_canvas_x,_canvas_y))
                    time.sleep_ms(500)
                if (_s_daemon.get_status() == 2):
                    canvas.draw_rectangle(10,200, 230, 50, color=(0,0,0), thickness=1, fill=True)

                    display_cjk_string(canvas, 10,200, "Record OK.", font_size=2, color=(10,255,0))
                    # canvas.draw_string(10,200, "Record OK.", color=(10,255,0), scale=3, mono_space=False)
                    lcd.display(canvas, oft=(_canvas_x,_canvas_y))
        ...
        canvas.clear()
        display_cjk_string(canvas, 10,60, "Voice Sample", color=(10,255,0), font_size=2)
        display_cjk_string(canvas, 10,100, "Recording", color=(10,255,0), font_size=2)
        display_cjk_string(canvas, 10,140, "Successfull!", color=(10,255,0), font_size=2)
    ...
    display_cjk_string(canvas, 10,30, "Voice Sample", color=(255,0,0), font_size=2)
    display_cjk_string(canvas, 10,70, "Recording", color=(255,0,0), font_size=2)
    display_cjk_string(canvas, 10,110, "Failed!", color=(255,0,0), font_size=2)


    display_cjk_string(canvas, 10,150, "Reason: " + str(e), color=(255,0,0), font_size=2)
    lcd.display(canvas, oft=(_canvas_x,_canvas_y))
    _led_blue.value(0)
    _led_red.value(1)
